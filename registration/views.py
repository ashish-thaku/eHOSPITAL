...
class DoctorDashboardView(LoginRequiredMixin, DoctorRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # include doctor object so template can show doctor's details (name, specialty, etc.)
        doctor = Doctor.objects.get(user=self.request.user)
        context['doctor'] = doctor
        # provide a quick total patients count for template use
        try:
            context['total_patients'] = context['patients'].count()
        except Exception:
            context['total_patients'] = len(context.get('patients', []))
        return context
